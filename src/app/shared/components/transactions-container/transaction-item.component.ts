...
import { TransactionDetailsComponent } from '@client/secure/features/transactions/components/transaction-details/transaction-details.component';
...
@Component({
  ...
  template: `
    <div (click)="openDetails()" class="item flex gap-2 justify-between items-stretch pt-4"
    [ngClass]="!isLastItem() ? ' pb-4 border-b border-[#F2F2F2] dark:border-[#F2F2F2]/10' : ''"
    >
      ...
      <div class="main-details flex flex-col justify-start gap-1 w-[179px]">
        <p class="description text-sm line-clamp-2 leading-[18px]" appDarkable="dark:text-(color:--dm-secondary)">{{ transaction().description }}</p>
        <div class="targets flex gap-[0.625rem] justify-start items-center">
          ...
          @let source_currency_code = transaction().source ? this.financeStore.currenciesMap()[transaction().source?.currency_id!].code : null;
          @let destination_currency_code = transaction().destination !== null ? this.financeStore.currenciesMap()[transaction().destination?.currency_id!].code : null;

          @switch (transaction().type) {
            @case ('g2g') {
              <p class="origin text-xs text-(color:--secondary)/60" appDarkable="dark:text-(color:--dm-secondary)"> {{ origin_account_name! | wordReplacer:'Banco ': '' }} </p>
              @if(origin_account_name !== destination_account_name) {
                ...
                <p class="destination text-xs text-(color:--secondary)/60" appDarkable="dark:text-(color:--dm-secondary)"> {{ destination_account_name! | wordReplacer:'Banco ': '' }} </p>
              }
            }
            @case ('income') {
      ...
      <div class="secondary-details flex flex-col justify-between items-end w-full max-w-[82px]">
        <div class="currency flex gap-[5px] justify-end items-center">
          @if(source_currency_code !== destination_currency_code && (source_currency_code && destination_currency_code)) {
            <p class="origin text-xs font-medium text-(color:--secondary)" appDarkable="dark:text-(color:--dm-secondary)"> {{ source_currency_code }} </p>
            <svg width="5" height="7" viewBox="0 0 5 7" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M1 0.5L4 3.5L1 6.5" stroke="gray" stroke-opacity="0.8" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            <p class="origin text-xs font-medium text-(color:--secondary)" appDarkable="dark:text-(color:--dm-secondary)"> {{ destination_currency_code }} </p>
          } @else {
            <p class="destination text-xs font-medium text-(color:--secondary)" appDarkable="dark:text-(color:--dm-secondary)"> {{ source_currency_code ?? destination_currency_code }} </p>
          }
        </div>
        <div class="amount">
          <p class="text-xs font-bold" [style.color]="transaction() | transactionColor">
            {{ transaction() | transactionPrefix }}{{ transaction() | transactionExchange | money }}
          </p>
        </div>
        <div class="date">
    ...
    <q-drawer [(visible)]="showDetails">
      <ng-template #panel>
          <app-transaction-details [transaction]="transaction()" [financeStore]="financeStore"></app-transaction-details>
      </ng-template>
    </q-drawer>
  `,
  styles: ``
...
export class TransactionItemComponent {
  ...
  openDetails(): void {
    this.showDetails.update(val => val = true);
  }
}
