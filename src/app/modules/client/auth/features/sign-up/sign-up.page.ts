  ...
  template: `
    ...
    class="panel-section limited-container flex flex-col gap-5 items-center">
      ...
      <div class="form-container w-full">
        <form [formGroup]="signUpForm" (ngSubmit)="onSubmit()" class="flex flex-col gap-5 w-full">
...
export class SignUpPage {
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      if (error) {
        PopupService.error(error.message || 'Erro ao criar a conta. Tente novamente.');
      } else {
        if (data.session) {
          PopupService.success('Conta criada com sucesso!');
          this.router.navigate(['/secure/dashboard']);
        } else {
          PopupService.success('Conta criada! Por favor, verifique o seu email para confirmar o registo.');
          this.router.navigate(['/auth/sign-in']);
        }
      }
    } catch (err: any) {
      PopupService.error('Ocorreu um erro ao comunicar com o servidor.');
    } finally {
